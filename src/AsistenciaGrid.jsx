...
// Estilos por tipo, se sobreescriben con los colores de criterios
const TIPO_DEFAULT = {
  A:  { bg: '#dcfce7', color: '#15803d' },
  MD: { bg: '#fef3c7', color: '#92400e' },
  D:  { bg: '#f3f4f6', color: '#6b7280' },
  B:  { bg: '#fee2e2', color: '#b91c1c' },
}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
      ...
      <div style={{ background: '#fff', borderRadius: '14px', padding: '24px', width: '100%', maxWidth: '440px', boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
        ...
        <div style={{ marginTop: '8px', padding: '12px', background: '#f8fafc', borderRadius: '8px', fontSize: '12px', color: '#64748b' }}>
          <strong>D = Descanso</strong> (domingo). Los domingos se marcan automáticamente. Las horas de D no cuentan en el total.
        </div>
...
function CellPicker({ tipos, onSelect, onClose, anchorRef }) {
  ...
  return (
    <div ref={ref} style={{
      ...
      padding: '8px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px',
      minWidth: '120px', top: '100%', left: '50%', transform: 'translateX(-50%)',
    }}>
      {tipos.map(t => (
...
export default function AsistenciaGrid() {
  ...
  const [gridData, setGridData]         = useState(null)   // { generacion, fechas, alumnos }
  const [loading, setLoading]           = useState(false)
  const [error, setError]               = useState('')
  const [criterios, setCriterios]       = useState([])
  const [showCriterios, setShowCriterios] = useState(false)
  const [activeCell, setActiveCell]     = useState(null)   // { alumnoIdx, fecha }
  const [saving, setSaving]             = useState({})     // { 'alumnoIdx_fecha': true }
  ...
  const cargar = useCallback(async () => {
    ...
    try { setGridData(await getAsistenciaGrid(genId)) }
    catch (e) { setError(e.message) }
  ...
  // Fallback para tipos sin criterio
  for (const [k, v] of Object.entries(TIPO_DEFAULT)) {
    if (!tipoStyle[k]) tipoStyle[k] = { bg: v.bg, color: v.color, label: k }
  }

  // Cambio de celda (optimista)
  async function handleCellChange(alumnoIdx, fecha, tipo) {
    setActiveCell(null)
    const alumno = gridData.alumnos[alumnoIdx]
    const prevTipo = alumno.dias[fecha]?.tipo

    // Horas del nuevo tipo
    const horasCrit = criterios.find(c => c.Tipo === tipo)?.Horas ?? 0
    const horasPrev = alumno.dias[fecha]?.horas ?? 0

    // Actualización optimista
    setGridData(prev => {
      const alumnos = prev.alumnos.map((a, i) => {
        if (i !== alumnoIdx) return a
        const prevCnt = { cntA: a.cntA, cntMD: a.cntMD, cntB: a.cntB }
        if (prevTipo === 'A') prevCnt.cntA = Math.max(0, prevCnt.cntA - 1)
        if (prevTipo === 'MD') prevCnt.cntMD = Math.max(0, prevCnt.cntMD - 1)
        if (prevTipo === 'B') prevCnt.cntB = Math.max(0, prevCnt.cntB - 1)
        if (tipo === 'A') prevCnt.cntA++
        if (tipo === 'MD') prevCnt.cntMD++
        if (tipo === 'B') prevCnt.cntB++
        return {
          ...a,
          dias: { ...a.dias, [fecha]: { tipo, horas: Number(horasCrit), esManual: true } },
          totalHoras: a.totalHoras - horasPrev + Number(horasCrit),
          ...prevCnt,
        }
      })
      return { ...prev, alumnos }
  ...
  function exportarExcel() {
    ...
    const rows = alumnos.map((a, i) => [
      ...
      a.totalHoras, a.cntA, a.cntMD, a.cntB,
    ])

    const ws = XLSX.utils.aoa_to_sheet([header, ...rows])
    const colWidths = [{ wch: 4 }, { wch: 18 }, { wch: 34 }, ...fechas.map(() => ({ wch: 7 })), { wch: 9 }, { wch: 5 }, { wch: 5 }, { wch: 5 }]
    ws['!cols'] = colWidths
                ...
                <tr style={{ background: '#f8fafc' }}>
                  ...
                  {fechas.map(f => {
                    ...
                    return (
                      <th key={f} style={{
                        ...thDate,
                        background: esDom ? '#f1f5f9' : esSab ? '#f8fafc' : '#fff',
                        color: esDom ? '#94a3b8' : '#374151',
                        borderLeft: '1px solid #f1f5f9',
                      }}>
                        <div style={{ fontWeight: 700 }}>{d.getDate()}</div>
                        <div style={{ fontSize: '10px', color: esDom ? '#94a3b8' : '#94a3b8', fontWeight: 400 }}>{DIAS_SEMANA[d.getDay()]}</div>
                      </th>
                    )
                  })}
                  {/* Totales */}
                  <th style={thTotals}>Total hrs</th>
                  <th style={{ ...thTotals, color: '#15803d' }}>A</th>
                  <th style={{ ...thTotals, color: '#92400e' }}>MD</th>
                  <th style={{ ...thTotals, color: '#b91c1c' }}>B</th>
                </tr>
              </thead>
              <tbody>
                {alumnos.map((alumno, ai) => (
                  <tr key={alumno.OperadorId} style={{ borderTop: '1px solid #f1f5f9' }}
                    ...
                    {fechas.map(f => {
                      ...
                      return (
                        <td key={f} style={{
                          ...
                          background: esDom ? '#f8fafc' : undefined,
                    ...
                    <td style={{ ...tdTotal, color: '#15803d', fontWeight: 700 }}>{alumno.cntA}</td>
                    <td style={{ ...tdTotal, color: '#92400e', fontWeight: 700 }}>{alumno.cntMD}</td>
                    <td style={{ ...tdTotal, color: '#b91c1c', fontWeight: 700 }}>{alumno.cntB}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const thTotals = {
  ...
  background: '#f8fafc', position: 'sticky', right: 0,
}
